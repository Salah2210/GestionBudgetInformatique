    ...
    return(
        ...
        <div className="container">
          ...
          <table className={"table table-bordred table-hover table-striped"}>
              ...
              <tbody>
              {
               tadlaouardighas.map((elem,index)=>{
                return (
                <tr key={index}>
                  <td>{index+1}</td>
                  <td >{elem.code} </td>
                  <Link to={`/tadlaouardigha/edit/${elem._id}`}><td>{elem.branche}</td></Link>
                  <td>{elem.direg}</td>
                  <td>{elem.type}</td>
                  <td>{elem.classf}</td>
                  <td>{elem.bpt}</td>
...
export default TadlaOuardighaDir*/
...
function TadlaOuardighaDir() {
  ...
  useEffect(() => {
    ...
    ws.current.onmessage = (event) => {
      ...
      if (message.type === "update_TadlaOuardigha") {
  ...
  return (
    <div>
      ...
      <p>
        <b>Direction Régionale  Tadla Ouardigha </b>
      </p>
      <div className="notificationn-header">
        <Form>
      ...
      <div className="table-responsive">
        <table className="table table-bordered align-middle custom-table">
          ...
          <tbody>
            {paginatedTadlaouardighas.map((elem, index) => {
              return (
                <tr key={index}>
                  <td colSpan={2} className="truncate">
                    <Link to={`/tadlaouardigha/edit/${elem._id}`}>
                      {elem.branche}
                    </Link>
                  </td>
      ...
      <div className="d-flex justify-content-center mt-3">
        ...
        <button
          ...
          onClick={() => setPage(page + 1)}
          disabled={endIndex >= filteredTadlaouardighas.length}
        >
          Next
        </button>
      </div>
