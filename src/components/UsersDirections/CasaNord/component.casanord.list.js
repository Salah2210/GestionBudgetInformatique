...
import { deleteCasaNord, getAllCasaNords } from "../../../services/directions/casanord_service";
import DirectionsLayout from "../../component.directions.layout";
//import AdminLayout from "../../admin/component.admin.layout";

import '../../../pages/dashboard/Dashboard.css'; // Assuming you have a CSS file for styling

function CasaNordDir(){
   ...
   useEffect(()=>{
    ...
    ws.onmessage = (event) => {
      ...
      const notification = { ...message1, expiry: new Date().getTime() + 60000 }; // Set expiry time 1 minute from now
...
///////
  async function getCasaNords(){
    const result=await getAllCasaNords()
    setCasaNords(result.data)
  }
    ...
    return(
        ...
        {notifications.map((notification, index) => (
            ...
            <div>Data:
              <ul>
               ...
               <li><strong>Direction Régionale: {notification.data.direg}</strong></li>
                ...
                <li><strong>Comment: {notification.data.coment}</strong></li>
                </ul>
            </div>
          </div>
        ))}
          ...
          <p><b>Direction Régionale Casa NORD</b></p>
          <table className={"table table-bordred table-hover table-striped"}>
              ...
              <tbody>
              {
               casanords.map((elem,index)=>{
                return (
                <tr key={index}>
                  <td>{index+1}</td>
                  <td >{elem.code} </td>
                  <Link to={`/casanord/edit/${elem._id}`}><td>{elem.branche}</td></Link>
                  <td>{elem.direg}</td>
                  <td>{elem.type}</td>
                  <td>{elem.classf}</td>
                  <td>{elem.bpt}</td>
...
function CasaNordDir() {
  ...
  useEffect(() => {
    ...
    ws.current.onmessage = (event) => {
      ...
      if (message.type === "update_casanord") {
  ...
  const filteredCasanords = casanords.filter((elem) => {
    return search.toLowerCase() === "" || elem.branche.toLowerCase().includes(search.toLowerCase());
  });

  const startIndex = (page - 1) * perPage;
  const endIndex = startIndex + perPage;
  const paginatedCasanords = filteredCasanords.slice(startIndex, endIndex);
  ...
  return (
    <div>
      ...
      <p>
        <b>Direction Régionale Casa NORD</b>
      </p>
      <div className="notificationn-header">
        <Form>
      ...
      <div className="table-responsive">
        <table className="table table-bordered align-middle custom-table">
          ...
          <tbody>
            {paginatedCasanords.map((elem, index) => {
              return (
                <tr key={index}>
                  <td colSpan={2} className="truncate">
                    <Link to={`/casanord/edit/${elem._id}`}>
                      {elem.branche}
                    </Link>
                  </td>
      ...
      <div className="d-flex justify-content-center mt-3">
        ...
        <button
          ...
          onClick={() => setPage(page + 1)}
          disabled={endIndex >= filteredCasanords.length}
        >
          Next
        </button>
      </div>
