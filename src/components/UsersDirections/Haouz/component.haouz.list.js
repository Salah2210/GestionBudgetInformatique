    ...
    return(
        ...
        <div className="container">
          ...
          <table className={"table table-bordred table-hover table-striped"}>
              ...
              <tbody>
              {
               haouzs.map((elem,index)=>{
                return (
                <tr key={index}>
                  <td>{index+1}</td>
                  <td >{elem.code} </td>
                  <Link to={`/haouz/edit/${elem._id}`}><td>{elem.branche}</td></Link>
                  <td>{elem.direg}</td>
                  <td>{elem.type}</td>
                  <td>{elem.classf}</td>
                  <td>{elem.bpt}</td>
    ...
    <div>
      ...
      <div className="table-responsive">
        <table className="table table-bordered align-middle custom-table">
          ...
          <tbody>
            {paginatedHaouzs.map((elem, index) => {
              return (
                <tr key={index}>
                  <td colSpan={2} className="truncate">
                    <Link to={`/haouz/edit/${elem._id}`}>
                      {elem.branche}
                    </Link>
                  </td>
      ...
      <div className="d-flex justify-content-center mt-3">
        ...
        <button
          ...
          onClick={() => setPage(page + 1)}
          disabled={endIndex >= filteredHaouzs.length}
        >
          Next
        </button>
      </div>
