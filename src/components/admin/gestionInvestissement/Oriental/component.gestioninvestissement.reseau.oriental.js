...
function OrientalAdmin(){
  ...
  async function delOriental(id){
          await deleteOriental(id)
          getOrientals()
  }
 ...
 const paginatedOrientals = filteredOrientals.slice(startIndex, endIndex);
    ...
    return(
        ...
        <div>
           ...
           <Link className="mt-3 btn btn-primary" to={"/admin/gestioninvestissements/reseau/oriental/new"}>Ajouter</Link>
          {/**/}
          ...
          <div className="table-responsive">
              <table className="table table-bordered align-middle custom-table">
                  ...
                  <tbody>
                  {paginatedOrientals.filter((elem) => {
                      return search.toLowerCase() === ''
                          ? elem
                      ...
                      return (
                          <tr key={index}>
                              <td colSpan={2} className="truncate"><Link to={`/admin/gestioninvestissements/reseau/oriental/edit/${elem._id}`}>{elem.branche}</Link></td>
          ...
          <div className="d-flex justify-content-center">
              {Array.from({ length: Math.ceil(orientals.length / perPage) }, (_, i) => (
