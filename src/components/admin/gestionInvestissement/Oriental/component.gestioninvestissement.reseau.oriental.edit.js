...
import { getOrientalById, updateOriental } from "../../../../services/directions/oriental_service"


function OrientalEdit(){
    ...
    async function getOriental(){
        const rep= await getOrientalById(id)
    ...
    async function submitOriental(e){
        e.preventDefault()
        const oriental={"_id":id,"code":code,"branche":branche,"direg":direg,"type":type,"classf":classf,
        ...
        await updateOriental(oriental)
        navigate("/admin/gestioninvestissements/reseau/oriental")
    }

    return (
        <div className="container" id="form-contact">
            <h2>{branche}</h2>
            <form onSubmit={(e)=> submitOriental(e)}>
            ...
            <input className="m-2 btn btn-primary" type={"submit"} value={"Enregistrer"}/>
             <button style={{backgroundColor:'#130f40',marginBottom:'15px'}} className="m-2" to='admin/gestioninvestissements/reseau/oriental'>&lt;Back</button>
