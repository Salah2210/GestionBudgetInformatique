...
import { deleteTadlaOuardigha, getAllTadlaOuardighas } from "../../../../services/directions/tadlaouardigha_service";
import Form from 'react-bootstrap/Form';
import InputGroup from 'react-bootstrap/InputGroup';
import '../CasaSudAdmin.css'; // Import the CSS file
import AdminLayout from "../../component.admin.layout";

function TadlaOuardighaAdmin(){
  ...
  async function getTadlaOuardighas(){
    const result=await getAllTadlaOuardighas()
    setTadlaOuardighas(result.data)
  }

  async function delTadlaOuardigha(id){
          await deleteTadlaOuardigha(id)
          getTadlaOuardighas()
  }

  const filteredTadlaouardighas = tadlaouardighas.filter((elem) => {
 ...
 const paginatedTadlaouardighas = filteredTadlaouardighas.slice(startIndex, endIndex);
    return(
        
        <div>
                    <AdminLayout/>

           <Link className="mt-3 btn btn-primary" to={"/admin/gestioninvestissements/reseau/tadlaouardigha/new"}>Ajouter</Link>
          {/**/}
          ...
          <p><b>Direction Régionale Tadla Ouardigha Zayane</b></p>
          <Form>
          ...
          <div className="table-responsive">
              <table className="table table-bordered align-middle custom-table">
                  ...
                  <tbody>
                  {paginatedTadlaouardighas.filter((elem) => {
                      return search.toLowerCase() === ''
                          ? elem
                      ...
                      return (
                          <tr key={index}>
                              <td colSpan={2} className="truncate"><Link to={`/admin/gestioninvestissements/reseau/tadlaouardigha/edit/${elem._id}`}>{elem.branche}</Link></td>
          ...
          <div className="d-flex justify-content-center">
              {Array.from({ length: Math.ceil(tadlaouardighas.length / perPage) }, (_, i) => (
...
export default TadlaOuardighaAdmin