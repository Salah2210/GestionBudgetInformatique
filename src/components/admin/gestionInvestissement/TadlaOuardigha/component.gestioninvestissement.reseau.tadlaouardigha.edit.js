...
import { getTadlaOuardighaById, updateTadlaOuardigha } from "../../../../services/directions/tadlaouardigha_service"


function TadlaOuardighaEdit(){
    ...
    async function getTadlaOuardigha(){
        const rep= await getTadlaOuardighaById(id)
    ...
    async function submitTadlaOuardigha(e){
        e.preventDefault()
        const tadlaouardigha={"_id":id,"code":code,"branche":branche,"direg":direg,"type":type,"classf":classf,
        ...
        await updateTadlaOuardigha(tadlaouardigha)
        navigate("/admin/gestioninvestissements/reseau/tadlaouardigha")
    }

    return (
        <div className="container" id="form-contact">
            <h2>{branche}</h2>
            <form onSubmit={(e)=> submitTadlaOuardigha(e)}>
            ...
            <input className="m-2 btn btn-primary" type={"submit"} value={"Enregistrer"}/>
             <button style={{backgroundColor:'#130f40',marginBottom:'15px'}} className="m-2" to='admin/gestioninvestissements/reseau/tadlaouardigha'>&lt;Back</button>
...
export default TadlaOuardighaEdit
