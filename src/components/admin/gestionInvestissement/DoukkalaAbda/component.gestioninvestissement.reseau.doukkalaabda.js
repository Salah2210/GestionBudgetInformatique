...
import { deleteDoukkalaAbda, getAllDoukkalaAbdas } from "../../../../services/directions/doukkalaabda_service";
import '../CasaSudAdmin.css'; // Import the CSS file
import AdminLayout from "../../component.admin.layout";

function DoukkalaAbdaAdmin(){
  ...
  async function getDoukkalaAbdas(){
    const result=await getAllDoukkalaAbdas()
    setDoukkalaAbdas(result.data)
  }

  async function delDoukkalaAbda(id){
          await deleteDoukkalaAbda(id)
          getDoukkalaAbdas()
  }
  const filteredDoukkalaabdas = doukkalaabdas.filter((elem) => {
 ...
 const paginatedDoukkalaabdas = filteredDoukkalaabdas.slice(startIndex, endIndex);
    ...
    return(
        ...
        <div >
           ...
           {/*<Link className="mt-3 btn btn-primary" to={"/admin/gestioninvestissements/reseau/doukkalaabda/new"}>Ajouter</Link>
          */}
          ...
          <p><b>Direction Régionale Doukkala Abda</b></p>
          <Form>
          ...
          <div className="table-responsive">
              <table className="table table-bordered align-middle custom-table">
                  ...
                  <tbody>
                  {paginatedDoukkalaabdas.filter((elem) => {
                      return search.toLowerCase() === ''
                          ? elem
                      ...
                      return (
                          <tr key={index}>
                              <td colSpan={2} className="truncate"><Link to={`/admin/gestioninvestissements/reseau/doukkalaabda/edit/${elem._id}`}>{elem.branche}</Link></td>
          ...
          <div className="d-flex justify-content-center">
              {Array.from({ length: Math.ceil(doukkalaabdas.length / perPage) }, (_, i) => (
...
export default DoukkalaAbdaAdmin