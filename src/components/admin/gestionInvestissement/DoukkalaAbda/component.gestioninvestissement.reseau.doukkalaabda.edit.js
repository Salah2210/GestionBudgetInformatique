...
import { getDoukkalaAbdaById, updateDoukkalaAbda } from "../../../../services/directions/doukkalaabda_service"


function DoukkalaAbdaEdit(){
    ...
    async function getDoukkalaAbda(){
        const rep= await getDoukkalaAbdaById(id)
    ...
    async function submitDoukkalaAbda(e){
        e.preventDefault()
        const doukkalaabda={"_id":id,"code":code,"branche":branche,"direg":direg,"type":type,"classf":classf,
        ...
        await updateDoukkalaAbda(doukkalaabda)
        navigate("/admin/gestioninvestissements/reseau/doukkalaabda")
    }

    return (
        <div className="container" id="form-contact">
            <h2>{branche}</h2>
            <form onSubmit={(e)=> submitDoukkalaAbda(e)}>
            ...
            <input className="m-2 btn btn-primary" type={"submit"} value={"Enregistrer"}/>
             <button style={{backgroundColor:'#130f40',marginBottom:'15px'}} className="m-2" to='admin/gestioninvestissements/reseau/doukkalaabda'>&lt;Back</button>
...
export default DoukkalaAbdaEdit
