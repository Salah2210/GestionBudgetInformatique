...
import { getHaouzById, updateHaouz } from "../../../../services/directions/haouz_service"
...
function HaouzEdit(){
    ...
    async function getHaouz(){
        const rep= await getHaouzById(id)
    ...
    async function submitHaouz(e){
        e.preventDefault()
        const haouz={"_id":id,"code":code,"branche":branche,"direg":direg,"type":type,"classf":classf,
        ...
        await updateHaouz(haouz)
        navigate("/admin/gestioninvestissements/reseau/haouz")
    }

    return (
        <div className="container" id="form-contact">
            <h2>{branche}</h2>
            <form onSubmit={(e)=> submitHaouz(e)}>
            ...
            <input className="m-2 btn btn-primary" type={"submit"} value={"Enregistrer"}/>
             <button style={{backgroundColor:'#130f40',marginBottom:'15px'}} className="m-2" to='admin/gestioninvestissements/reseau/haouz'>&lt;Back</button>
