...
import { deleteNord, getAllNords } from "../../../../services/directions/nord_service";
...
function NordAdmin(){
  ...
  async function getNords(){
    const result=await getAllNords()
    setNords(result.data)
  }

  async function delNord(id){
          await deleteNord(id)
          getNords()
  }
  const filteredNords = nords.filter((elem) => {
 ...
 const paginatedNords = filteredNords.slice(startIndex, endIndex);
    ...
    return(
        ...
        <div>
          ...
          {/* <Link className="mt-3 btn btn-primary" to={"/admin/gestioninvestissements/reseau/nord/new"}>Ajouter</Link>
          */}
          ...
          <p><b>Direction Régionale Nord</b></p>
          <Form>
          ...
          <div className="table-responsive">
              <table className="table table-bordered align-middle custom-table">
                  ...
                  {paginatedNords.filter((elem) => {
                      ...
                      return (
                          <tr key={index}>
                              <td colSpan={2} className="truncate"><Link to={`/admin/gestioninvestissements/reseau/nord/edit/${elem._id}`}>{elem.branche}</Link></td>
          ...
          <div className="d-flex justify-content-center">
              {Array.from({ length: Math.ceil(nords.length / perPage) }, (_, i) => (
