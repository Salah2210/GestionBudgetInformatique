...
import { deleteRabatTemara, getAllRabatTemaras } from "../../../../services/directions/rabattemara_service";
import Form from 'react-bootstrap/Form';
import InputGroup from 'react-bootstrap/InputGroup';
import '../CasaSudAdmin.css'; // Import the CSS file
import AdminLayout from "../../component.admin.layout";

function RabatTemaraAdmin(){
  ...
  async function getRabatTemaras(){
    const result=await getAllRabatTemaras()
    setRabatTemaras(result.data)
  }

  async function delRabatTemara(id){
          await deleteRabatTemara(id)
          getRabatTemaras()
  }

  const filteredRabattemaras = rabattemaras.filter((elem) => {
 ...
 const paginatedRabattemaras = filteredRabattemaras.slice(startIndex, endIndex);
    return(
        
        <div>
                    <AdminLayout/>

           <Link className="mt-3 btn btn-primary" to={"/admin/gestioninvestissements/reseau/rabattemara/new"}>Ajouter</Link>
          {/**/}
          ...
          <p><b>Direction Régionale Rabat Temara</b></p>
          <Form>
          ...
          <div className="table-responsive">
              <table className="table table-bordered align-middle custom-table">
                  ...
                  <tbody>
                  {paginatedRabattemaras.filter((elem) => {
                      return search.toLowerCase() === ''
                          ? elem
                      ...
                      return (
                          <tr key={index}>
                              <td colSpan={2} className="truncate"><Link to={`/admin/gestioninvestissements/reseau/rabattemara/edit/${elem._id}`}>{elem.branche}</Link></td>
          ...
          <div className="d-flex justify-content-center">
              {Array.from({ length: Math.ceil(rabattemaras.length / perPage) }, (_, i) => (
...
export default RabatTemaraAdmin