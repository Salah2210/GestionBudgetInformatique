...
import { getRabatTemaraById, updateRabatTemara } from "../../../../services/directions/rabattemara_service"


function RabatTemaraEdit(){
    ...
    async function getRabatTemara(){
        const rep= await getRabatTemaraById(id)
    ...
    async function submitRabatTemara(e){
        e.preventDefault()
        const rabattemara={"_id":id,"code":code,"branche":branche,"direg":direg,"type":type,"classf":classf,
        ...
        await updateRabatTemara(rabattemara)
        navigate("/admin/gestioninvestissements/reseau/rabattemara")
    }

    return (
        <div className="container" id="form-contact">
            <h2>{branche}</h2>
            <form onSubmit={(e)=> submitRabatTemara(e)}>
            ...
            <input className="m-2 btn btn-primary" type={"submit"} value={"Enregistrer"}/>
             <button style={{backgroundColor:'#130f40',marginBottom:'15px'}} className="m-2" to='admin/gestioninvestissements/reseau/rabattemara'>&lt;Back</button>
...
export default RabatTemaraEdit
