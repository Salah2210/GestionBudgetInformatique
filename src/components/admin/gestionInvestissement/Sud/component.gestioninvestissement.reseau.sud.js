...
import { deleteSud, getAllSuds } from "../../../../services/directions/sud_service";
...
function SudAdmin(){
  ...
  async function getSuds(){
    const result=await getAllSuds()
    setSuds(result.data)
  }

  async function delSud(id){
          await deleteSud(id)
          getSuds()
  }
  const filteredSuds = suds.filter((elem) => {
 ...
 const paginatedSuds = filteredSuds.slice(startIndex, endIndex);
    ...
    return(
        ...
        <div>
           ...
           <Link className="mt-3 btn btn-primary" to={"/admin/gestioninvestissements/reseau/sud/new"}>Ajouter</Link>
          {/**/}
          ...
          <p><b>Direction Régionale Sud</b></p>
          <Form>
          ...
          <div className="table-responsive">
              <table className="table table-bordered align-middle custom-table">
                  ...
                  {paginatedSuds.filter((elem) => {
                      ...
                      return (
                          <tr key={index}>
                              <td colSpan={2} className="truncate"><Link to={`/admin/gestioninvestissements/reseau/sud/edit/${elem._id}`}>{elem.branche}</Link></td>
          ...
          <div className="d-flex justify-content-center">
              {Array.from({ length: Math.ceil(suds.length / perPage) }, (_, i) => (
