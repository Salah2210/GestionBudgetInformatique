...
function CasaNordAdmin(){
  ...
  async function delCasaNord(id){
          await deleteCasaNord(id)
          getCasaNords()
  }
 ...
 const paginatedCasanords = filteredCasanords.slice(startIndex, endIndex);
    ...
    return(
        ...
        <div >
          <AdminLayout/>
            {/*<Link className="mt-3 btn btn-primary" to={"/admin/gestioninvestissements/reseau/casanord/new"}>Ajouter</Link>
         */}
          ...
          <thead className="bg-light">
                  ...
                  <tbody>
                  {/*casanords*/paginatedCasanords.filter((elem) => {
                      return search.toLowerCase() === ''
                          ? elem
                      ...
                      return (
                          <tr key={index}>
                              <td colSpan={2} className="truncate"><Link to={`/admin/gestioninvestissements/reseau/casanord/edit/${elem._id}`}>{elem.branche}</Link></td>
          ...
          <div className="d-flex justify-content-center">
              {Array.from({ length: Math.ceil(casanords.length / perPage) }, (_, i) => (
