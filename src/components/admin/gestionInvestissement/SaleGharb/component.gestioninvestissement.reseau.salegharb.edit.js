...
import { getSaleGharbById, updateSaleGharb } from "../../../../services/directions/salegharb_service"


function SaleGharbEdit(){
    ...
    async function getSaleGharb(){
        const rep= await getSaleGharbById(id)
    ...
    async function submitSaleGharb(e){
        e.preventDefault()
        const salegharb={"_id":id,"code":code,"branche":branche,"direg":direg,"type":type,"classf":classf,
        ...
        await updateSaleGharb(salegharb)
        navigate("/admin/gestioninvestissements/reseau/salegharb")
    }

    return (
        <div className="container" id="form-contact">
            <h2>{branche}</h2>
            <form onSubmit={(e)=> submitSaleGharb(e)}>
            ...
            <input className="m-2 btn btn-primary" type={"submit"} value={"Enregistrer"}/>
             <button style={{backgroundColor:'#130f40',marginBottom:'15px'}} className="m-2" to='admin/gestioninvestissements/reseau/salegharb'>&lt;Back</button>
