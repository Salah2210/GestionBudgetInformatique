...
import { getCasaSudById, updateCasaSud } from "../../../../services/directions/casasud_service"


function CasaSudEdit(){
    ...
    async function getCasaSud(){
        const rep= await getCasaSudById(id)
    ...
    async function submitCasaSud(e){
        e.preventDefault()
        const casasud={"_id":id,"code":code,"branche":branche,"direg":direg,"type":type,"classf":classf,
        ...
        await updateCasaSud(casasud)
        navigate("/admin/gestioninvestissements/reseau/casasud")
    }

    return (
        <div className="container" id="form-contact">
            <h2>{branche}</h2>
            <form onSubmit={(e)=> submitCasaSud(e)}>
            ...
            <input className="m-2 btn btn-primary" type={"submit"} value={"Enregistrer"}/>
             <button style={{backgroundColor:'#130f40',marginBottom:'15px'}} className="m-2" to='admin/gestioninvestissements/reseau/casasud'>&lt;Back</button>
