...
import { getFesMeknesById, updateFesMeknes } from "../../../../services/directions/fesmeknes_service"


function FesMeknesEdit(){
    ...
    async function getFesMeknes(){
        const rep= await getFesMeknesById(id)
    ...
    async function submitFesMeknes(e){
        e.preventDefault()
        const fesmeknes={"_id":id,"code":code,"branche":branche,"direg":direg,"type":type,"classf":classf,
        ...
        await updateFesMeknes(fesmeknes)
        navigate("/admin/gestioninvestissements/reseau/fesmeknes")
    }

    return (
        <div className="container" id="form-contact">
            <h2>{branche}</h2>
            <form onSubmit={(e)=> submitFesMeknes(e)}>
            ...
            <input className="m-2 btn btn-primary" type={"submit"} value={"Enregistrer"}/>
             <button style={{backgroundColor:'#130f40',marginBottom:'15px'}} className="m-2" to='admin/gestioninvestissements/reseau/fesmeknes'>&lt;Back</button>
